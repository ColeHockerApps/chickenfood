...
struct FoodPlayView: UIViewRepresentable {

    let startPoint: URL
    let launch: FoodLaunchPoints
    let orientation: FoodOrientationManager
    let onReady: () -> Void
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: FoodLaunchPoints
    private let orientation: FoodOrientationManager
    ...
    init(
        startPoint: URL,
        launch: FoodLaunchPoints,
        orientation: FoodOrientationManager,
    ...
    func webView(
        ...
        let urlStr = action.request.url?.absoluteString ?? "nil"
        let scheme = action.request.url?.scheme?.lowercased() ?? "nil"
        let host = action.request.url?.host?.lowercased() ?? "nil"
        let isMain = (webView === mainView)
        ...
        guard let next = action.request.url,
              ...
              nextScheme == "http" || nextScheme == "https" || nextScheme == "about"
        else {
    ...
    private func scheduleSaveIfNeeded() {
        guard didScheduleSave == false else {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) { [weak self] in
            guard let self,
                  let active = self.mainView?.url else {
            ...
            let changed = (self.normalize(now) != self.normalize(base))
    ...
    private func runMarksJob(for point: URL, in view: WKWebView) {
        ...
        marksTimer = Timer.scheduledTimer(withTimeInterval: 10, repeats: true) { [weak view, weak launch] _ in
            guard let view, let launch else {
